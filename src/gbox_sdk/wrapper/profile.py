...
class Logger:
    ...
    def __init__(self, error=None, warn=None, info=None, debug=None):
    ...
    def _validate_and_transform_config(self, parsed: Any) -> Optional[ProfileConfig]:
        ...
        try:
            ...
            if 'current' in parsed:
                config.current = str(parsed['current'])
            
            # Extract profiles
            if 'profiles' in parsed and isinstance(parsed['profiles'], dict):
                for key, profile in parsed['profiles'].items():
                    if profile and isinstance(profile, dict) and 'key' in profile:
                        ...
                        org_name = profile_data.get('org_name') or profile_data.get('org', '')
                        config.profiles[key] = ProfileData(
                            org_name=org_name,
